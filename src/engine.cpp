...
void SetScreen(SDL_Surface *screen){
	...
	//Nie mo¿na wskaŸnikowi tego przypisaæ ;/
}
...
void InitGame(){
	if(SDL_Init(SDL_INIT_VIDEO)==-1){
		std::cerr<<"B³¹d inicjalizacji: SDL_Init(SDL_INIT_VIDEO) :"<<SDL_GetError()<<std::endl;
		exit(1);
	}
	if(TTF_Init()!=0){
		std::cerr<<"B³¹d inicjalizacji TTF_Init() :"<<TTF_GetError()<<std::endl;
		SDL_Quit();
		exit(1);
	}
	//std::cout<<"Pomyœlnie zainicjalizowano SDL!"<<std::endl;
	SDL_ShowCursor(SDL_DISABLE);
	LoadImages();
	//std::cout<<"Pomyœlnie wczytano grafikê!"<<std::endl;
}
...
void ClearGame(){
	...
	SDL_Quit();
	//TIME <ctime>
	//std::cout<<"Pamiêæ zosta³a zwolniona pomyœlnie zwolniona!"<<std::endl;
}
